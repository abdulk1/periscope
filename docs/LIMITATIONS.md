...
- **An auto-update check.** Not built. It would be this application's first
  outbound network call, and the security posture allows exactly one (opt-in,
  never silent). There is also nowhere to check yet: no release is published.
